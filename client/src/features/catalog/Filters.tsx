...
export default function Filters({filtersData}: Props) {
    ...
    return (
        <Box display='flex' flexDirection='column' gap={3}>
            ...
            <Paper sx={{p: 3}}>
                <CheckboxButtons items={filtersData.brands} checked={brands} onChange={(items: string[]) => dispatch(setBrands(items))} />
            </Paper>
            <Paper sx={{p: 3}}>
                <CheckboxButtons items={filtersData.types} checked={types} onChange={(items: string[]) => dispatch(setTypes(items))} />
            </Paper>
